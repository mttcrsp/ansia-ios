...
struct FacesDetectionClient {
  ...
  var perform: (Request) throws -> Response

  init(facesObservationsClient: FaceObservationsClient = .init()) {
    perform = { request in
      let observations = try facesObservationsClient
        .faceObservations(request.cgImage, request.minimumConfidence)
        .filter { $0.confidence >= request.minimumConfidence }

      guard !observations.isEmpty else {
        throw Error.noFaceFound
      }

      let originalSize = CGSize(width: request.cgImage.width, height: request.cgImage.height)
      let boundingRects = observations.map(\.boundingBox)
