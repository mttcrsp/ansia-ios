...
public struct FacesDetectionClient {
  ...
  public var perform: (Request, @escaping (Result<Response, Swift.Error>) -> Void) -> Void

  public init(facesObservationsClient: FaceObservationsClient = .init()) {
    perform = { request, completion in
      facesObservationsClient
        .faceObservations(request.cgImage) { result in
          switch result {
